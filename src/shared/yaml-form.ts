...
export const ACTION_OPTIONS: { action: string; label: string; fields: ActionField[] }[] = [
  {
    action: 'aiTap',
    label: '点击',
    fields: [{ key: 'locate', label: '点击什么', placeholder: '如：登录按钮', type: 'text' }],
  },
  {
    action: 'aiInput',
    label: '输入',
    fields: [
      { key: 'locate', label: '在哪个输入框', placeholder: '如：用户名输入框', type: 'text' },
      { key: 'value', label: '输入内容', placeholder: '如：{{USERNAME}}', type: 'text' },
    ],
  },
  {
    action: 'aiAssert',
    label: '断言验证',
    fields: [{ key: 'prompt', label: '验证什么', placeholder: '如：页面显示登录成功', type: 'text' }],
  },
  {
    action: 'aiWaitFor',
    label: '等待出现',
    fields: [
      { key: 'prompt', label: '等待什么出现', placeholder: '如：页面跳转到首页', type: 'text' },
      { key: 'timeout', label: '超时(毫秒)', placeholder: '15000', type: 'number', optional: true },
    ],
  },
  {
    action: 'ai',
    label: '自由指令',
    fields: [{ key: 'prompt', label: '让 AI 做什么', placeholder: '如：关闭所有弹窗', type: 'text' }],
  },
  {
    action: 'aiHover',
    label: '悬停',
    fields: [{ key: 'locate', label: '悬停在哪里', placeholder: '如：更多菜单', type: 'text' }],
  },
  {
    action: 'aiRightClick',
    label: '右键点击',
    fields: [{ key: 'locate', label: '右键点击什么', placeholder: '如：文件图标', type: 'text' }],
  },
  {
    ...
    fields: [
      {
        ...
        label: '按哪个键',
        type: 'select',
        options: ['Enter', 'Tab', 'Escape', 'Backspace', 'ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight'].map((key) => ({
          label: key,
          value: key,
        })),
      },
  ...
  },
  {
    ...
    fields: [
      ...
      { key: 'distance', label: '距离(像素)', placeholder: '留空自动', type: 'number', optional: true },
    ],
  },
  {
    action: 'aiQuery',
    label: '提取数据',
    fields: [{ key: 'prompt', label: '提取什么', placeholder: '如：{title: string} 页面标题', type: 'text' }],
  },
  {
    action: 'sleep',
    label: '固定等待',
...
function stepToYamlObject(step: FormStep) {
  ...
  switch (step.action) {
    case 'aiInput':
      body = { aiInput: { locate: params.locate ?? '', value: params.value ?? '' } };
      break;
    case 'aiScroll': {
      const scroll: Record<string, unknown> = { direction: params.direction ?? 'down' };
...
function yamlStepToForm(step: unknown): FormStep | null {
  ...
  const formStep = (params: FormStep['params']): FormStep => ({ id: createStepId(), action, name, params });
  ...
  switch (action) {
    ...
    case 'aiKeyboardPress': {
      const key = typeof raw === 'string' ? raw : isRecord(raw) && typeof raw.key === 'string' ? raw.key : null;
      return key === null ? null : formStep({ key });
    }
    case 'aiInput': {
      if (!isRecord(raw) || typeof raw.locate !== 'string' || (typeof raw.value !== 'string' && typeof raw.value !== 'number')) {
        return null;
      }
...
export function yamlToForm(yamlText: string): YamlToFormResult {
  ...
  if (!isRecord(doc) || typeof doc.target !== 'string' || !Array.isArray(doc.tasks) || doc.tasks.length === 0) {
    return { ok: false };
  }
