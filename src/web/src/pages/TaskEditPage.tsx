...
import { App as AntApp, Alert, Button, Card, Flex, Input, InputNumber, Segmented, Select, Space, Typography } from 'antd';
...
function createEmptyStep(action = 'aiTap'): FormStep {
  return { id: createStepId(), action, params: action === 'aiScroll' ? { direction: 'down' } : action === 'aiKeyboardPress' ? { key: 'Enter' } : {} };
}

function createEmptyTask(index: number): FormTask {
  return { id: createStepId(), name: `步骤组 ${index}`, steps: [createEmptyStep()] };
}
...
export default function TaskEditPage() {
  ...
  const updateTask = (index: number, patch: Partial<FormTask>) => {
    setForm((prev) => ({ ...prev, tasks: prev.tasks.map((task, i) => (i === index ? { ...task, ...patch } : task)) }));
  };

  const updateStep = (taskIndex: number, stepIndex: number, patch: Partial<FormStep>) => {
    const task = form.tasks[taskIndex];
    updateTask(taskIndex, { steps: task.steps.map((step, i) => (i === stepIndex ? { ...step, ...patch } : step)) });
  };
  ...
  const save = async () => {
    ...
    try {
      ...
      if (isNew) {
        ...
        await api.updateTask(Number(id), { name: name.trim(), yaml: yamlToSave });
      }
      message.success('已保存');
      navigate('/tasks');
    } catch (error) {
  ...
  const renderStepFields = (taskIndex: number, stepIndex: number, step: FormStep) => {
    ...
    return option.fields.map((field) => {
      ...
      const onChange = (next: string | number | undefined) =>
        updateStep(taskIndex, stepIndex, { params: { ...step.params, [field.key]: next } });
    ...
    <Space orientation="vertical" size="middle" style={{ width: '100%' }}>
      <Flex gap={12} wrap="wrap" align="center">
        ...
        <InputNumber
          ...
          onChange={(value) => setForm((prev) => ({ ...prev, viewportWidth: value ?? undefined }))}
        />
        <InputNumber
          placeholder="视口高(默认800)"
          min={320}
          value={form.viewportHeight}
          onChange={(value) => setForm((prev) => ({ ...prev, viewportHeight: value ?? undefined }))}
        />
      </Flex>
          ...
          extra={
            <Space>
              <Button
                ...
                onClick={() => setForm((prev) => ({ ...prev, tasks: moveItem(prev.tasks, taskIndex, -1) }))}
              />
              <Button
                size="small"
              ...
              <Button
                ...
                onClick={() => setForm((prev) => ({ ...prev, tasks: prev.tasks.filter((_, i) => i !== taskIndex) }))}
              />
            </Space>
          }
        >
            ...
            {task.steps.map((step, stepIndex) => (
              <Flex key={step.id} gap={8} align="center" wrap="wrap">
                ...
                <Select
                  style={{ width: 110 }}
                  value={step.action}
                  options={ACTION_OPTIONS.map((option) => ({ label: option.label, value: option.action }))}
                  onChange={(action) => updateStep(taskIndex, stepIndex, createEmptyStep(action))}
                />
                ...
                <Input
                  ...
                  placeholder="步骤名(可选)"
                  value={step.name ?? ''}
                  onChange={(e) => updateStep(taskIndex, stepIndex, { name: e.target.value })}
                />
                <Button
                  size="small"
                  icon={<ArrowUpOutlined />}
                  disabled={stepIndex === 0}
                  onClick={() => updateTask(taskIndex, { steps: moveItem(task.steps, stepIndex, -1) })}
                />
                <Button
                  size="small"
                  icon={<ArrowDownOutlined />}
                ...
                <Button
                  ...
                  onClick={() => updateTask(taskIndex, { steps: task.steps.filter((_, i) => i !== stepIndex) })}
                />
              </Flex>
            ))}
            <Button
              type="dashed"
              icon={<PlusOutlined />}
              onClick={() => updateTask(taskIndex, { steps: [...task.steps, createEmptyStep()] })}
            >
              添加步骤
            </Button>
          </Flex>
      ...
      <Button
        ...
        block
        icon={<PlusOutlined />}
        onClick={() => setForm((prev) => ({ ...prev, tasks: [...prev.tasks, createEmptyTask(prev.tasks.length + 1)] }))}
      >
        添加步骤组
      </Button>
    </Space>
  ...
  return (
    <Card
      ...
      extra={
        <Space>
          <Segmented value={mode} onChange={switchMode} options={[{ label: '表单编辑', value: 'form' }, { label: 'YAML 编辑', value: 'yaml' }]} />
    ...
    >
      <Space orientation="vertical" size="middle" style={{ width: '100%' }}>
        ...
        {errors.length === 0 && loaded && <Alert type="success" title="校验通过" showIcon />}
      </Space>
    </Card>
  );
}
