...
import type { RunRecord, RunStatus, RunStepRecord, TaskRecord } from '../shared/types';
...
function toTask(row: TaskRow): TaskRecord {
  return { id: row.id, name: row.name, yaml: row.yaml, createdAt: row.created_at, updatedAt: row.updated_at };
}
...
export function updateTask(db: Database, id: number, input: { name: string; yaml: string }) {
  db.prepare('UPDATE tasks SET name = ?, yaml = ?, updated_at = ? WHERE id = ?').run(input.name, input.yaml, now(), id);
}
...
export function listTasks(db: Database) {
  const rows = db.prepare('SELECT * FROM tasks ORDER BY updated_at DESC, id DESC').all() as TaskRow[];
...
export function insertRun(db: Database, input: { taskId: number | null; taskName: string; model: string }) {
  const result = db
    .prepare("INSERT INTO runs (task_id, task_name, model, status, started_at) VALUES (?, ?, ?, 'running', ?)")
...
export function finishRun(
  db: Database,
  id: number,
  input: { status: RunStatus; error: string | null; durationMs: number; tokenInput: number; tokenOutput: number },
) {
  db.prepare(
    'UPDATE runs SET status = ?, error = ?, finished_at = ?, duration_ms = ?, token_input = ?, token_output = ? WHERE id = ?',
  ).run(input.status, input.error, now(), input.durationMs, input.tokenInput, input.tokenOutput, id);
}
...
export function listRunSteps(db: Database, runId: number) {
  const rows = db.prepare('SELECT * FROM run_steps WHERE run_id = ? ORDER BY step_index ASC').all(runId) as StepRow[];
...
export function listVariables(db: Database) {
  const rows = db.prepare('SELECT key, value FROM kv WHERE key LIKE ? ORDER BY key').all(`${VARIABLE_PREFIX}%`) as {
    key: string;
    value: string;
  }[];
  return Object.fromEntries(rows.map((row) => [row.key.slice(VARIABLE_PREFIX.length), row.value]));
}
...
export function getSetting(db: Database, key: string) {
  ...
  const row = db.prepare('SELECT value FROM kv WHERE key = ?').get(settingKey) as { value: string } | null;
