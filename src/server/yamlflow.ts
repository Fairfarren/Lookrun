...
function validateStep(
    ...
    if (action === 'aiScroll') {
        if (
            !isRecord(params) ||
            !['up', 'down', 'left', 'right'].includes(String(params.direction))
        ) {
...
export function parseScript(yamlText: string, variables: Record<string, string>): ParseResult {
    ...
    try {
        ...
        errors.push(
            `YAML 语法错误：${error instanceof Error ? error.message.split('\n')[0] : String(error)}`,
        );
    ...
    doc.tasks.forEach((task: unknown, taskIndex: number) => {
        ...
        task.flow.forEach((step: unknown, stepIndex: number) => {
            ...
            if (isRecord(step)) {
                const actionKeys = Object.keys(step).filter(
                    (key) => key !== 'name' && key !== 'timeout' && key !== 'url',
                );
                const action = actionKeys[0];
                flow.push({
                    action,
