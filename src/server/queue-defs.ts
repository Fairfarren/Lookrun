...
export function listQueues(db: Database): QueueDef[] {
  const rows = db.prepare('SELECT * FROM queues ORDER BY updated_at DESC, id DESC').all() as (QueueDef & Record<string, string>)[];
  return rows.map((row) => ({
    id: row.id,
    name: row.name,
...
export function createQueue(db: Database, input: { name: string }): QueueDef {
  const time = now();
  const result = db.prepare('INSERT INTO queues (name, created_at, updated_at) VALUES (?, ?, ?)').run(input.name, time, time);
  const row = db.prepare('SELECT * FROM queues WHERE id = ?').get(Number(result.lastInsertRowid)) as Record<string, string>;
...
export function getQueue(db: Database, id: number): QueueDefWithItems | null {
  const qRow = db.prepare('SELECT * FROM queues WHERE id = ?').get(id) as Record<string, string> | null;
  if (!qRow) {
    return null;
  }
  const itemRows = db.prepare('SELECT * FROM queue_items WHERE queue_id = ? ORDER BY position').all(id) as Record<string, string>[];
  return {
    id: Number(qRow.id),
    name: qRow.name,
...
export function updateQueue(db: Database, id: number, input: { name: string; items: { taskId: number; modelId: string }[] }) {
  const time = now();
  ...
  input.items.forEach((item, index) => {
    db.prepare('INSERT INTO queue_items (queue_id, task_id, model_id, position, created_at) VALUES (?, ?, ?, ?, ?)').run(
      id,
      item.taskId,
      item.modelId,
      index + 1,
