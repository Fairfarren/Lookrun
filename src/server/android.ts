...
export function createAndroidAppLauncher(input: AndroidAppLauncherInput) {
	return async (target: string) => {
		...
		throw new Error(
			`设备的应用列表中没有找到名为「${value}」的 App，请检查名称或改填准确包名`,
		);
	};
}

export function createAndroidDeviceLister(
	provider: () => Promise<AndroidDeviceInfo[]>,
) {
	return async () => {
...
export function createAndroidDeviceChecker(
	listDevices: () => Promise<AndroidDeviceOption[]>,
) {
...
export function resolveAdbPath(input: ResolveAdbPathInput) {
	...
	const candidates = [
		...
		input.sdkRoot
			? pathApi.join(input.sdkRoot, "platform-tools", adbName)
			: undefined,
		input.pathAdb,
	];
	return candidates.find(
		(candidate): candidate is string => Boolean(candidate && input.exists(candidate)),
	) ?? null;
}
...
export function detectAdbPath() {
	...
	return resolveAdbPath({
		...
		pathAdb:
			Bun.which(process.platform === "win32" ? "adb.exe" : "adb") ?? undefined,
		exists: existsSync,
	});
}
