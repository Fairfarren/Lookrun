...
export function enqueue(
    ...
    const max = db.prepare('SELECT COALESCE(MAX(position), 0) AS max FROM run_queue').get() as {
        max: number;
    };
...
export function listQueue(db: Database) {
    const rows = db
        .prepare("SELECT * FROM run_queue WHERE status IN ('pending', 'running') ORDER BY position")
        .all() as QueueRow[];
...
export function nextPending(db: Database) {
    const row = db
        .prepare("SELECT * FROM run_queue WHERE status = 'pending' ORDER BY position LIMIT 1")
        .get() as QueueRow | null;
    return row ? toQueueItem(row) : null;
}

export function setQueueStatus(db: Database, id: number, status: QueueStatus, runId?: number) {
    db.prepare('UPDATE run_queue SET status = ?, run_id = COALESCE(?, run_id) WHERE id = ?').run(
        status,
        runId ?? null,
        id,
    );
...
export function moveQueueItem(db: Database, id: number, direction: 'up' | 'down') {
    const current = db.prepare('SELECT * FROM run_queue WHERE id = ?').get(id) as QueueRow | null;
    ...
    const neighbor = db.prepare(neighborQuery).get(current.position) as QueueRow | null;
    if (!neighbor) {
        return;
    }
    db.prepare('UPDATE run_queue SET position = ? WHERE id = ?').run(neighbor.position, current.id);
    db.prepare('UPDATE run_queue SET position = ? WHERE id = ?').run(current.position, neighbor.id);
}

export function cancelQueueItem(db: Database, id: number) {
    db.prepare("UPDATE run_queue SET status = 'cancelled' WHERE id = ? AND status = 'pending'").run(
        id,
    );
}
...
export function requeueInterrupted(db: Database) {
    db.prepare("UPDATE run_queue SET status = 'pending' WHERE status = 'running'").run();
}
