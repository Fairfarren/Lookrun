...
import { formToYaml, yamlToForm, type FormScript } from '../src/shared/yaml-form';
  ...
  tasks: [
    {
      ...
      name: '登录',
      steps: [
        { id: 's1', action: 'aiInput', name: '输账号', params: { locate: '用户名输入框', value: '{{USERNAME}}' } },
        { id: 's2', action: 'aiTap', params: { locate: '登录按钮' } },
        { id: 's3', action: 'aiWaitFor', params: { prompt: '跳转到首页', timeout: 10000 } },
      ],
    },
    {
      id: 't2',
      name: '验证',
      steps: [
        { id: 's4', action: 'ai', params: { prompt: '检查一下页面有没有报错' } },
        { id: 's5', action: 'aiAssert', params: { prompt: '页面显示登录成功' } },
        { id: 's6', action: 'aiScroll', params: { direction: 'down', distance: 300 } },
...
describe('formToYaml', () => {
  ...
  test('视口留空时不生成视口字段', () => {
    const yaml = formToYaml({ ...fullForm, viewportWidth: undefined, viewportHeight: undefined });
  ...
  test('特殊字符（冒号、引号、#）被正确转义', () => {
    const form: FormScript = {
      target: 'https://a.com',
      tasks: [{ id: 't', name: '组', steps: [{ id: 's', action: 'aiAssert', params: { prompt: '标题是 "A: #1"' } }] }],
    };
...
describe('yamlToForm', () => {
  test('表单生成的 YAML 可以无损解析回表单', () => {
    ...
    expect(form.tasks[0].steps[1]).toMatchObject({ action: 'aiTap', params: { locate: '登录按钮' } });
    expect(form.tasks[0].steps[0].name).toBe('输账号');
    expect(form.tasks[0].steps[2].params).toMatchObject({ prompt: '跳转到首页', timeout: 10000 });
    expect(form.tasks[1].steps[2].params).toMatchObject({ direction: 'down', distance: 300 });
