...
import { countRuns, createDb, insertRun, insertStep } from '../src/server/db';
import { cleanupAllRuns, dirSizeBytes, storageStats } from '../src/server/storage';
...
describe('storageStats', () => {
  test('分项返回截图/报告/数据库占用与运行数', () => {
    ...
    expect(stats.totalBytes).toBe(stats.screenshotsBytes + stats.reportsBytes + stats.databaseBytes);
...
describe('cleanupAllRuns', () => {
  test('清空运行记录与步骤、删除截图和报告目录内容，任务不受影响', () => {
    ...
    insertStep(db, {
      runId: run.id, stepIndex: 0, stepName: 's', action: 'aiTap', url: null, prompt: null,
      aiResult: null, shotBefore: null, shotAfter: null, durationMs: 1,
      tokenInput: 0, tokenOutput: 0, status: 'success', error: null,
    });
    writeFile('screenshots/1/0-before.jpg', 500);
    writeFile('midscene-report/r1/index.html', 300);
    db.prepare('INSERT INTO tasks (name, yaml, created_at, updated_at) VALUES (?, ?, ?, ?)').run('任务', 'yaml', 't', 't');

    const result = cleanupAllRuns(db, path.join(tempDir, 'screenshots'), path.join(tempDir, 'midscene-report'));

    expect(result.deletedRuns).toBe(1);
    expect(result.freedBytes).toBe(800);
    expect(countRuns(db)).toBe(0);
    expect(db.prepare('SELECT COUNT(*) AS c FROM run_steps').get()).toEqual({ c: 0 });
    // 任务还在
