...
describe("parseModelsConfig", () => {
	...
	test("models 为空数组报错", () => {
		expect(() =>
			parseModelsConfig({ baseUrl: "u", apiKey: "k", models: [] }),
		).toThrow();
	});
...
describe("parseVisionCheckResponse", () => {
	...
	test("bbox 超出 1000 且不在图片范围则失败", () => {
		expect(parseVisionCheckResponse('{"bbox": [0, 0, 5000, 5000]}').ok).toBe(
			false,
		);
	});
	...
	test("归一化坐标的负值或超 1000 则失败", () => {
		expect(parseVisionCheckResponse('{"bbox": [-5, 100, 200, 300]}').ok).toBe(
			false,
		);
		expect(parseVisionCheckResponse('{"bbox": [100, 200, 1200, 300]}').ok).toBe(
			false,
		);
	});

	test("bbox 面积为零则失败", () => {
		expect(parseVisionCheckResponse('{"bbox": [10, 10, 10, 10]}').ok).toBe(
			false,
		);
	});
