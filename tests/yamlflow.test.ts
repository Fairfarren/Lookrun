...
import { parseScript, substituteVariables } from '../src/server/yamlflow';

describe('substituteVariables', () => {
  test('替换 {{变量}} 占位符', () => {
    const text = 'value: "{{USERNAME}} / {{PASSWORD}}"';
    expect(substituteVariables(text, { USERNAME: 'alice', PASSWORD: 's3cret' })).toBe('value: "alice / s3cret"');
  });
  ...
  test('没有占位符时原样返回', () => {
    expect(substituteVariables('target: https://a.com', { A: '1' })).toBe('target: https://a.com');
  });
});
...
`, {});
  ...
  test('target 不是 http(s) 地址报错', () => {
    const result = parseScript('target: ftp://a.com\ntasks:\n  - name: a\n    flow:\n      - ai: xxx\n', {});
    expect(result.ok).toBe(false);
  });
