...
import { createDb, insertTask } from '../src/server/db';
import {
  createQueue,
  deleteQueue,
  getQueue,
...
beforeEach(() => {
  ...
  insertTask(db, { name: '任务A', yaml: 'target: https://a.com\ntasks:\n  - name: a\n    flow:\n      - ai: x' });
  insertTask(db, { name: '任务B', yaml: 'target: https://b.com\ntasks:\n  - name: b\n    flow:\n      - ai: x' });
});
...
describe('队列 CRUD', () => {
  ...
  test('更新队列名和条目（全量替换）', () => {
    ...
    expect(got?.items[0]).toMatchObject({ taskId: 1, modelId: 'kimi', position: 1 });
    expect(got?.items[1]).toMatchObject({ taskId: 2, modelId: 'gemma4', position: 2 });
  });

  test('再次更新替换旧条目', () => {
    const q = createQueue(db, { name: 'q' });
    updateQueue(db, q.id, { name: 'q', items: [{ taskId: 1, modelId: 'kimi' }] });
    updateQueue(db, q.id, { name: 'q', items: [{ taskId: 2, modelId: 'gemma4' }] });
