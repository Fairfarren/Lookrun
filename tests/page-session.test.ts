...
describe("resolvePageSessionAction", () => {
	...
	test("空字符串 url 时留在当前页", () => {
		expect(
			resolvePageSessionAction("  ", "https://h5.example.com", [
				"https://h5.example.com",
			]),
		).toEqual({ type: "stay" });
	});
	...
	test("填写当前页同一地址时不重新打开", () => {
		...
		expect(
			resolvePageSessionAction("https://h5.example.com", current, [current]),
		).toEqual({ type: "stay" });
	});
	...
	test("填写已打开过的其他地址时复用", () => {
		...
		expect(
			resolvePageSessionAction("https://h5.example.com/", admin, [h5, admin]),
		).toEqual({ type: "reuse", key: h5 });
	});

	test("填写从未打开过的地址时新开", () => {
		const h5 = pageSessionKey("https://h5.example.com");
		expect(
			resolvePageSessionAction("https://admin.example.com", h5, [h5]),
		).toEqual({
			type: "open",
