  ...
  executions: [
    {
      ...
      tasks: [
        { type: 'Planning', thought: '需要先找到登录按钮' },
        {
          type: 'Insight',
          thought: '找到了登录按钮',
          output: { element: { rect: { left: 10, top: 20, width: 100, height: 40 }, center: [60, 40] } },
        },
      ],
    },
  ],
...
describe('extractLastAiResult', () => {
  ...
  test('提取断言结果', () => {
    const dump = JSON.stringify({
      executions: [{ name: 'aiAssert', tasks: [{ type: 'Insight', output: { pass: true, thought: '页面确实有欢迎语' } }] }],
    });
    expect(extractLastAiResult(dump)).toMatchObject({ assertPass: true, thought: '页面确实有欢迎语' });
  });

  test('提取 aiQuery 的数据', () => {
    const dump = JSON.stringify({
      executions: [{ name: 'aiQuery', tasks: [{ type: 'Insight', output: { data: { title: 'Example' } } }] }],
    });
